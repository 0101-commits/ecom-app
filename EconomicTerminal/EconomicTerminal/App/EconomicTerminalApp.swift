...
//  앱의 시작점(@main)입니다. SwiftData 저장소를 만들고, 알림 델리게이트와
//  백그라운드 태스크를 연결한 뒤, 5개 탭의 루트 화면을 띄웁니다.
...
struct EconomicTerminalApp: App {
    ...
    init() {
        ...
        // 백그라운드 알림 점검 태스크 등록(앱 종료 전 반드시 등록).
        BackgroundRefreshManager.shared.configure(container: modelContainer)
    }
